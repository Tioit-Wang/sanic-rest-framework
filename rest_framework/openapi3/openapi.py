...
from rest_framework.openapi3.definitions import (
...
def secured(*args, **kwargs):
    raise NotImplementedError(
        "SecuritySchemas are not yet implemented in sanic-openapi 0.6.3, " "hopefully they should be ready for the next release."
    )
...
def definition(
    *,
    ...
    def inner(func):
        ...
        if tag:
            taglist = []
            op = "extend" if isinstance(tag, (list, tuple, set, frozenset)) else "append"

            getattr(taglist, op)(tag)
            glbl["tag"](*[tag.fields["name"] if isinstance(tag, Tag) else tag for tag in taglist])(func)
        ...
        if parameter:
            paramlist = []
            op = "extend" if isinstance(parameter, (list, tuple, set, frozenset)) else "append"
            ...
            for param in paramlist:
                ...
                if isinstance(param, Parameter):
                    ...
                    raise SanicException(
                        "parameter must be a Parameter instance, a string, or " "a dictionary containing at least 'name'."
                    )
        ...
        if response:
            resplist = []
            op = "extend" if isinstance(response, (list, tuple, set, frozenset)) else "append"
