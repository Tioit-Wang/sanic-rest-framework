...
from .types import Definition, Integer, Object, Schema, String
...
class Response(Definition):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(content=content, status=status, description=description, **kwargs)

    @staticmethod
    def make(content_data, description: str = None, wrap: bool = True, wrap_content=None, **kwargs):
        if wrap_content is None:
            wrap_content = {"code": Integer(example=1), "message": String(example="OK"), "data": None}
        if not description:
            description = "Default Response"
        formatted_content_data = {}
        for media_type, schema_data in content_data.items():
            if not isclass(schema_data):
                schema = Schema.make(schema_data)
            elif not issubclass(schema_data, Definition):
                schema = Schema.make(schema_data)
            else:
                schema = schema_data
            if wrap:
                current_wrap_content = wrap_content.copy()
                current_wrap_content["data"] = schema
                formatted_content_data[media_type] = Object(current_wrap_content)
            else:
                formatted_content_data[media_type] = schema
        return Response(MediaType.all(formatted_content_data), description=description, **kwargs)
