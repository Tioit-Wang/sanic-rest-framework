...
# from srf.serializers import BaseSerializer
...
class Definition:
    ...
    def serialize(self):
        return {
            ...
            if (v or (isinstance(self.__nullable__, list) and (not self.__nullable__ or k in self.__nullable__)))
        }
    ...
    def apply(self, func, operations, *args, **kwargs):
        op = operations[func]
        method_name = getattr(self.__class__, "__method__", self.__class__.__name__.lower())
...
def _properties(value: object) -> Dict:
    ...
    return {k: v for k, v in {**get_type_hints(cls), **fields}.items() if not k.startswith("_")}
