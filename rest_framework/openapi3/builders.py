...
from rest_framework.openapi3.autodoc import YamlStyleParametersParser
...
class SpecificationBuilder:
    ...
    _security_schemes: Dict[str, SecurityScheme]
    ...
    def _do_contact(self, name: str = None, url: str = None, email: str = None):
        if self._contact:
            return
    ...
    def _build_security(self):
        security_schemes = {}
        for api_path in self._paths.values():
            for api_method in api_path.values():
                for security in api_method.security:
                    name = security.fields.get("name", "")
                    if name and name not in security_schemes:
                        security_schemes[name] = security
    ...
    def build(self) -> OpenAPI:
        ...
        security_schemes = self._build_security()
        ...
        if security_schemes:
            if "securitySchemes" not in self._components:
                self._components["securitySchemes"] = {}
            self._components["securitySchemes"].update(security_schemes)

        components = Components(**self._components) if self._components else None
    ...
    def _build_paths(self) -> Dict:
        ...
        for path, operations in self._paths.items():
            paths[path] = PathItem(**{k: v if isinstance(v, dict) else v.build() for k, v in operations.items()})
